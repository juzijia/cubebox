...
//https://raw.githubusercontent.com/juzijia/quantumltX/refs/heads/main/list/template.js#type=组合订阅&name=FS&outbound=🕳ℹ️🐸 手动选择|♻️ 自动选择🏷ℹ️^(?!.*(?:官网|剩余|流量|套餐|免费|订阅|到期时间|全球直连|GB|Expire Date|Traffic|ExpireDate)).*🕳ℹ️📺 晚高峰专线🏷ℹ️.*(IPLC|iplc|IEPL|iepl|专线).*🕳ℹ️🇭🇰 香港故转🏷ℹ️^(?!.*(?:us)).*(🇭🇰|HK|hk|香港|港|HongKong)🕳ℹ️🇯🇵 日本故转🏷ℹ️^(?!.*(?:us)).*(🇯🇵|JP|jp|日本|日|Japan)🕳ℹ️🇸🇬 狮城故转🏷ℹ️^(?!.*(?:us)).*(新加坡|坡|狮城|SG|Singapore)🕳ℹ️🇨🇳 台湾故转🏷ℹ️^(?!.*(?:us)).*(🇹🇼|TW|tw|台湾|台|TaiWan)🕳ℹ️🇺🇲 美国故转🏷ℹ️^(?!.*(?:AUS|RUS)).*(🇺🇸|US|us|美国|美|United States)
// 示例说明
